<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('org.events.update', $event) }}" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @csrf
                    @method('PATCH')

                    <div class="mb-4 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Title</label>
                        <input id="event_title_input" type="text" name="title" value="{{ old('title', $event->title) }}" class="mt-1 block w-full border border-gray-200 rounded-md shadow-sm p-2 font-bold">
                    </div> 

                    <div class="mb-4 md:col-span-1">
                        <label class="block text-sm font-medium text-gray-700">Trail (optional)</label>
                        <select name="trail_id" id="trail_select" class="mt-1 block w-full border border-gray-200 rounded-md shadow-sm p-2">
                            <option value="" disabled>-- Select trail (optional) --</option>
                            @foreach($trails as $trail)
                                <option value="{{ $trail->id }}" data-duration="{{ optional($trail->package)->duration }}" @if(old('trail_id', $event->trail_id)==$trail->id) selected @endif>{{ $trail->trail_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4 md:col-span-1">
                        ...
                        <div id="duration_override_row" class="mt-3 hidden">
                            <input type="text" id="duration_override_input" name="duration" value="{{ old('duration', $event->duration ?? '') }}" class="mt-1 block w-full border border-gray-200 rounded-md shadow-sm p-2" placeholder="e.g. 2, 2:00, 2d" />
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="batch_count" value="{{ old('batch_count', $event->batch_count ?? '') }}" min="1" class="mt-1 block w-full border border-gray-200 rounded-md shadow-sm p-2" placeholder="e.g. 10" {{ empty(old('always_available', (empty($event->start_at) && empty($event->end_at)) ? '1' : '')) ? '' : 'disabled aria-disabled=true' }} />
                        <p class="text-xs text-gray-500 mt-1">If provided, the server will generate this many contiguous batches starting from the event start time using the duration (event duration override or trail package duration). Each batch will be a slot for bookings.</p>
                                        ...
                                        <div class="mb-4 flex items-start gap-3">
                                                <div class="flex items-center h-5">
                                                    ...
                                                    <input id="always_available" name="always_available" type="checkbox" value="1" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" @if(empty($event->start_at) && empty($event->end_at)) checked @endif />
                                                </div>
                                                <div class="text-sm">
                     ...
                     <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" name="start_at" value="{{ old('start_at', optional($event->start_at)->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full border border-gray-200 rounded-md shadow-sm p-2">
                        </div>

                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700">End</label>
                            <input type="datetime-local" name="end_at" value="{{ old('end_at', optional($event->end_at)->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full border border-gray-200 rounded-md shadow-sm p-2">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end space-x-3 md:col-span-2">
                        ...
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-[#336d66] text-white rounded">Save Event</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        (function(){
            ...
            function toggle() {
                ...
                if (start) {
                    ...
                    start.setAttribute('aria-disabled', disabled ? 'true' : 'false');
                    start.classList.toggle('opacity-50', disabled);
                    start.classList.toggle('cursor-not-allowed', disabled);
                }
                if (end) {
                    end.disabled = disabled;
                    end.setAttribute('aria-disabled', disabled ? 'true' : 'false');
                    end.classList.toggle('opacity-50', disabled);
                    end.classList.toggle('cursor-not-allowed', disabled);
                }
                try {
                    const batchCountInput = document.querySelector('input[name="batch_count"]');
                    if (batchCountInput) {
                        batchCountInput.disabled = disabled;
                        batchCountInput.setAttribute('aria-disabled', disabled ? 'true' : 'false');
                        batchCountInput.classList.toggle('opacity-50', disabled);
                        batchCountInput.classList.toggle('cursor-not-allowed', disabled);
                        if (disabled) batchCountInput.value = '';
                    }
                } catch (e) {
            ...
            toggleBtn?.addEventListener('click', function(){
                ...
                if (!overrideRow.classList.contains('hidden')) overrideInput?.focus();
                updatePreview();
            });

            trailSelect?.addEventListener('change', updatePreview);
            // ensure title is populated on submit if left empty (mirrors create behaviour)
            (function(){
            ...
            always?.addEventListener('change', toggle);
            toggle();
            updatePreview();
