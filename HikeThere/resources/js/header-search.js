// Simple debounced header search
document.addEventListener('DOMContentLoaded', function() {
    ...
    function iconHtmlForType(type) {
        // Using Font Awesome class names. Ensure FA is loaded in the layout.
        // Use fa-fw for fixed width and text-base for consistent sizing.
        if (type === 'trail') {
            return `<i class="fa-fw text-base fas fa-hiking text-green-600" aria-hidden="true"></i>`;
        }
        if (type === 'organization') {
            return `<i class="fa-fw text-base fas fa-users text-blue-600" aria-hidden="true"></i>`;
        }
        // location
        return `<i class="fa-fw text-base fas fa-map-marker-alt text-yellow-600" aria-hidden="true"></i>`;
    }
    ...
    function renderResults(results) {
        ...
        const ul = document.getElementById('header-search-results') || dropdown;
        ul.innerHTML = '';
    ...
    input.addEventListener('input', function(e) {
        ...
        timeout = setTimeout(() => {
            fetch(`/api/header-search?q=${encodeURIComponent(q)}`)
                .then(r => r.json())
